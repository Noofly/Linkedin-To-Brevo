...
2. Cliquer sur l'icône de l'extension (ou ⚙ dans le panneau) pour ouvrir les options :
   ...
   - **Tester la connexion**, puis **Vérifier les attributs** (crée les attributs contact manquants : `PRENOM`, `NOM`, `TITRE`, `SOCIETE`, `LANGUE`, `QUALIFIE`, `TELEPHONE_SOCIETE`, `LINKEDIN`).
...
2. Le panneau pré-remplit prénom, nom, titre, société, email, téléphone, site web, langue — corrigez si besoin puis **Créer dans Brevo**.
...
npm test
```
(Node ≥ 20, `node --test` sur les fonctions pures : normalisation, exploration de site, annuaire/BODACC, client Brevo.)
